...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Utilise angle=0 UNIQUEMENT si l'utilisateur dit 'tourne sans t'arrêter' "
            "ou 'continue de tourner'. Le robot tournera jusqu'au prochain stop_robot."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
class VoiceEngine:
    ...
    def _build_instructions(self) -> str:
        ...
        return (
            ...
            f"RÈGLES ABSOLUES :\n\n"
            f"1. LANGUE : Tu parles TOUJOURS en français. Si la transcription semble être dans une autre langue, c'est une erreur — devine ce que la personne voulait dire en français.\n\n"
            f"2. INTERLOCUTEURS : Tu parles avec des adultes ET des enfants (5 ans et plus). Les enfants prononcent moins bien et ont des voix aiguës. Fais un effort pour comprendre même si la transcription est approximative.\n\n"
            f"3. LONGUEUR : Réponds en 1 phrase courte maximum. Pas de question en retour.\n\n"
            f"4. CONVERSATION : Quand tu as répondu, tu attends qu'on te parle. Tu ne relances JAMAIS la conversation tout seul.\n\n"
            f"5. ACTIONS PHYSIQUES :\n"
            f"   - Quand on te demande de bouger (avance, recule, va à gauche/droite) → appelle move_robot IMMÉDIATEMENT.\n"
            f"   - Quand on te demande de tourner (tourne, demi-tour, tourne à gauche/droite) → appelle turn_robot IMMÉDIATEMENT.\n"
            f"   - Quand on te demande une action (danse, assis, couché, donne la patte, fais un cœur, étire-toi) → appelle do_action IMMÉDIATEMENT.\n"
            f"   - Quand on te dit '{stop_phrase}' ou 'stop' → appelle stop_robot.\n"
            f"   - Tu DOIS appeler la fonction correspondante quand on te donne un ordre. Ne te contente JAMAIS de dire que tu le fais sans appeler la fonction.\n"
            f"   - Ne fais aucune action de ta propre initiative (pas de danse, mouvement ou geste spontané quand personne ne te l'a demandé).\n"
