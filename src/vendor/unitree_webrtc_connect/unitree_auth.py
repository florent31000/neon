...
def make_local_request(path, body=None, headers=None):
    try:
        logging.info(f"HTTP POST to {path}")
        response = requests.post(url=path, data=body, headers=headers, timeout=10)

        response.raise_for_status()

        if response.status_code == 200:
            logging.info(f"HTTP 200 from {path}")
            return response
        else:
            logging.warning(f"HTTP {response.status_code} from {path}")
            return None

    except requests.exceptions.RequestException as e:
        logging.error(f"HTTP request to {path} failed: {e}")
        return None

# Function to send SDP to peer and receive the answer
def send_sdp_to_remote_peer(serial: str, sdp: str, access_token: str, public_key: Any) -> str:
    logging.info("Sending SDP to Go2...")
    from .encryption import aes_encrypt, generate_aes_key, rsa_encrypt, aes_decrypt
